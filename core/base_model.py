...
class BaseModel():
    ...
    def train(self):
        ...
        train_mse_losses = []
        eval_fid_losses = []
        with tqdm(initial=self.epoch, total=self.opt['train']['n_epoch']) as pbar:
...
//                        f'train_loss: {train_log["train/mse_loss"]:.4f},eval_fid_loss: {val_log["val/FID"].item():.4f},eval_lpips_loss: {val_log["val/LPIPS"].item():.4f}')
                ...
                if self.epoch % self.opt['train']['save_checkpoint_epoch'] == 0:
                    ...
                    path = os.path.join(self.opt['path']['checkpoint'], "last")
                ...
                if self.epoch % self.opt['train']['val_epoch'] == 0:
                    ...
                    if self.val_loader is None:
                        ...
                        eval_fid_losses.append(val_log['val/FID'].to('cpu').tolist())
                        pbar.set_description(
                            f'train_loss: {train_log["train/mse_loss"]:.4f},eval_fid_loss: {val_log["val/FID"].item():.4f},eval_lpips_loss: {val_log["val/LPIPS"].item():.4f}')
                        pbar.update(1)
                    ...
                    if val_log['val/FID'] < self.opt['train']['min_val_FID_loss']:
                        ...
                        path = os.path.join(self.opt['path']['checkpoint'], "best")
                    ...
                    if self.epoch == self.opt['train']['n_epoch']:
                        ...
                        self.opt['train']['eval_previous'].extend(eval_fid_losses)
